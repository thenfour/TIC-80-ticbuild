...
## Audio capture Lua API

This fork can capture the PCM generated by a running cartridge, as a 16-bit WAV:

```lua
audio_capture_start()                 -- true when capture starts
local state, detail = audio_capture_status()
audio_capture_end()                   -- true when capture begins stopping
```

`audio_capture_status()` returns one of `idle`, `capturing`, `stopping`,
`complete`, `error`, or `unsupported`. Its second return value is the output
filename for `complete`, the error message for `error`, and `nil` otherwise.

`audio_capture_end()` is asynchronous: capture includes the next synthesized
audio block, then becomes `complete`. The WAV is saved as `audio-capture.wav`
in the TIC-80 filesystem root. Captures are limited to 256mb.

Web builds also call one of these optional host hooks asynchronously:

```js
Module.onAudioCaptureComplete = ({ filename, mimeType, bytes }) => {};
